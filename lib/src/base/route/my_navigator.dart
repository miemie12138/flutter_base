...
class NavigatorManager {
  ...
  // 阻止到登录页面后的操作
  static ValueGetter<bool>? loginBlocksHandle;
  ...
  // subTabbar子项切换回调
  static ValueChanged<int>? subTabbarChanged;
  ...
  static Future<T?> push<T>(Widget page) async {
    ...
    if (loginBlocks.contains(page.runtimeType)) {
      final block = loginBlocksHandle?.call() ?? false;
  ...
  static void popToRoot([int tabIndex = -1, int subTabIndex = -1]) {
    if (tabIndex > -1) {
      tabbarChanged?.call(tabIndex);
      if (subTabIndex > -1) {
        Future.delayed(const Duration(milliseconds: 500), () {
          subTabbarChanged?.call(subTabIndex);
        });
      }
    }
...
class MyNavigator {
  ...
  // 退到首页
  static void popToRoot([int tabIndex = -1, int subTabIndex = -1]) {
    return NavigatorManager.popToRoot(tabIndex, subTabIndex);
  }
