...
class MyHttp {
  ...
  static Future<MyHttpResponse<T>> request<T>(
    ...
    // 附加数据
    final extraData = {"hideToast": hideToast, "hideErrorToast": hideErrorToast, "data": extra};
    final options = Options(extra: extraData, headers: headers);

    Response<dynamic> response;
    switch (type) {
      case MyHttpType.post:
        response = await dio
            .post(path, data: data, queryParameters: queryParams, options: options, cancelToken: cancelToken)
            .catchError((e) => throw e.error);
        break;
      case MyHttpType.put:
        response = await dio
            .put(path, data: data, queryParameters: queryParams, options: options, cancelToken: cancelToken)
            .catchError((e) => throw e.error);
        break;
      case MyHttpType.delete:
        response = await dio
            .delete(path, queryParameters: queryParams, options: options, cancelToken: cancelToken)
            .catchError((e) => throw e.error);
        break;
      default:
        response = await dio
            .get(path, queryParameters: queryParams, options: options, cancelToken: cancelToken)
            .catchError((e) => throw e.error);
    }
